...
def resolver_benchmark(vehiculos, capacidad, nodos):
    ...
    def distancia_euclidiana(from_index, to_index):
        ...
        return round(dist * FACTOR_ESCALA)
    ...
    def tiempo_callback(from_index, to_index):
        ...
        return round((n1["servicio"] + tiempo_viaje) * FACTOR_ESCALA)
    ...
    routing.AddDimension(time_callback_index, 10000000, 10000000, False, "Tiempo")

    time_dimension = routing.GetDimensionOrDie("Tiempo")
    for i in range(num_nodos):
        index = manager.NodeToIndex(i)
        time_dimension.CumulVar(index).SetRange(
            int(nodos[i]["inicio"] * FACTOR_ESCALA), int(nodos[i]["fin"] * FACTOR_ESCALA)
        )
...
if __name__ == "__main__":
    ...
    print("-" * 90)
    print(
        f"{'Instancia':<12} | {'Vehículos':<10} | {'Costo OR-Tools':<15} | {'Costo BKS':<12} | {'Gap %':<8} | {'Tiempo (s)':<12} | {'Factible'}"
    )
    print("-" * 90)
    ...
    for archivo_txt in archivos_txt:
        ...
        costo_ortools, vehiculos_usados, tiempo, factible = resolver_benchmark(
            vehiculos, capacidad, nodos
        )
        ...
        if factible:
            ...
            print(
                f"{nombre_instancia:<12} | {vehiculos_usados:<10} | {costo_ortools:<15.2f} | {bks_str:<12} | {gap_str:<8} | {tiempo:<12.2f} | {'Sí'}"
            )
        else:
            bks_str = f"{costo_referencia:.2f}" if costo_referencia else "N/A"
            print(
                f"{nombre_instancia:<12} | {'-':<10} | {'-':<15} | {bks_str:<12} | {'-':<8} | {tiempo:<12.2f} | {'No'}"
            )
