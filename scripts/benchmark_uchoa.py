...
def leer_instancia_uchoa(ruta_vrp):
    ...
    for linea in lineas:
        ...
        if seccion_actual == "COORD":
            ...
            nodos.append(
                {"id": int(datos[0]), "x": float(datos[1]), "y": float(datos[2]), "demanda": 0}
            )
...
if __name__ == "__main__":
    ...
    print("-" * 90)
    print(
        f"{'Instancia':<15} | {'Vehículos':<10} | {'Costo OR-Tools':<15} | {'Costo BKS':<12} | {'Gap %':<8} | {'Tiempo (s)':<12} | {'Factible'}"
    )
    print("-" * 90)
    ...
    for archivo_vrp in archivos_vrp:
        ...
        costo_ortools, vehiculos_usados, tiempo, factible = resolver_benchmark_cvrp(
        ...
        if factible:
            ...
            print(
                f"{nombre_instancia:<15} | {vehiculos_usados:<10} | {costo_ortools:<15.2f} | {bks_str:<12} | {gap_str:<8} | {tiempo:<12.2f} | {'Sí'}"
            )
        else:
            bks_str = f"{costo_referencia:.2f}" if costo_referencia else "N/A"
            print(
                f"{nombre_instancia:<15} | {'-':<10} | {'-':<15} | {bks_str:<12} | {'-':<8} | {tiempo:<12.2f} | {'No'}"
            )
