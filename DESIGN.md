...
- Un único acento de color por pantalla — el azul no compite consigo mismo.
- Español en toda la interfaz, sin excepción.

## Colors

Paleta restringida: neutrales + un acento azul de confianza, con verde reservado exclusivamente para estados de éxito/avance.

### Primary
- **Azul Confianza** (#2E5CFF): acción principal — botones primarios, foco de inputs, enlaces, el trazo de ruta en overlays. Es el único acento que aparece libremente; en pantallas de datos se usa con moderación.
- **Azul Confianza Hover** (#1A44E0): estado hover/press de todo lo que use el azul primario.
...
### Named Rules
**The One Accent Rule.** El azul primario es la única marca de color libre en una pantalla; el verde solo aparece cuando hay algo genuinamente completado o exitoso que señalar. Si ambos aparecen juntos sin ese motivo, el acento perdió su significado.
...
- **Sombra Botón Primario** (`0 4px 10px rgba(46,92,255,0.24)`): botones de acción principal.
...
- **Focus:** borde pasa a azul confianza + halo de `box-shadow` (`0 0 0 3px rgba(46,92,255,0.15)`) — nunca solo el outline nativo.
- **Error:** borde pasa a rojo peligro; el mensaje de error vive debajo del campo, no dentro.

### Overlay de Ruta (componente de firma)
SVG decorativo que traza una línea de ruta con un pin de inicio oscuro y un pin de destino en azul confianza con anillo de pulso — la misma convención visual que usa la pantalla operativa "Ruta Activa" del producto (oscuro = punto de partida, azul = destino). Aparece detrás de las pantallas de login/registro sobre la foto de fondo, animándose una sola vez al montar (trazo + aparición de pines + pulso continuo en el destino). No es decoración genérica: conecta el momento de iniciar sesión con la experiencia de ruta que el chofer va a usar el resto del día.

## Do's and Don'ts

### Do:
- **Do** usar el azul confianza como único acento libre por pantalla; el verde solo para éxito/avance real.
...
- **Do** dar a cada anillo de foco un tratamiento temático (azul confianza), nunca dejar el outline nativo del navegador sin tocar.
